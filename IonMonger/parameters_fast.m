function params = parameters_fast(iter)
    ...
    T     = 298;       % temperature (K)
    b     = cell2mat(iter(1));    % perovskite layer width (m) (normally between 150-600nm)
    epsp  = cell2mat(iter(2)); % permittivity of perovskite (Fm-1)
    alpha = cell2mat(iter(3));     % perovskite absorption coefficient (m-1)
    Ec    = -3.7;      % conduction band minimum (eV)
    Ev    = -5.4;      % valence band maximum (eV)
    Dn    = cell2mat(iter(4));    % perovskite electron diffusion coefficient (m2s-1)
    Dp    = cell2mat(iter(5));    % perovskite hole diffusion coefficient (m2s-1)
    gc    = 8.1e24;    % conduction band density of states (m-3)
    gv    = 5.8e24;    % valence band density of states (m-3)
    
    % Ion parameters
    N0    = cell2mat(iter(6));        % typical density of ion vacancies (m-3)
    ...
    DI    = cell2mat(iter(7));        % diffusion coefficient for iodide ions (m2s-1)
    ...
    dE    = cell2mat(iter(8));    % effective doping density of ETL (m-3)
    ...
    bE    = cell2mat(iter(9));  % width of ETL (m)
    epsE  = cell2mat(iter(10)); % permittivity of ETL (Fm-1)
    DE    = cell2mat(iter(11));    % electron diffusion coefficient in ETL (m2s-1)
    ...
    dH    = cell2mat(iter(12));    % effective doping density of HTL (m-3) 
    ...
    bH    = cell2mat(iter(13));  % width of HTL (m)
    epsH  = cell2mat(iter(14));  % permittivity of HTL (Fm-1)
    DH    = cell2mat(iter(15));    % hole diffusion coefficient in HTL (m2s-1)
    ...
    'linear', 1.2/1.0, 0, ... % reverse scan
    'linear', 1.2/1.0, 1.2 ... % forward scan
    };
